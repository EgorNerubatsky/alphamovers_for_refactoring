...
@foreach($privatChatMessages as $message)
    <div class="media media-reply">
        ...
        <div class="media-body bg-light">
            <div class="d-sm-flex justify-content-between mb-2">
                <h5 class="mb-sm-0">{{ $message->user->name }} {{ $message->user->lastname }}
                    @if($message->created_at != $message->updated_at)
                        <small
                            class="text-muted ml-3">{{$message->updated_at->format('d.m.Y')}} {{$message->updated_at->format('H:i')}}
                            (Кореговано)</small>
                    @else
                        <small
                ...
                <div class="media-reply__link">
                    ...
                    <a href="#" class="btn btn-transparent p-0 mr-3" data-toggle="modal"
                       ...
                       data-delete-url="{{ route($roleData['roleData']['messages_delete_message'], ['id'=>$message->id]) }}"
                       title="Видалити">
                    ...
                    <a href="#" data-toggle="modal" data-target="#myModal{{$message->id}}"
                       class="btn btn-transparent p-0 ml-3 font-weight-bold btn-edit">Вiдповiсти</a>
            ...
            @if(isset($message->reply_id))
                @php
                    $reply = Message::findOrFail($message->reply_id);
                @endphp

                <div class='card' style='color: #acbad5'>
                    <div class='card-body'>
                        {{$reply->user->name}} {{$reply->user->lastname}} ({{$reply->created_at->format('d.m.Y, H:i')}}):<br>{{$reply->message}}<br>
                    </div>
                </div>
            @endif
            <p>{!! $message->message !!}</p>
            ...
            @if($message->sender_user_id == $sender->id)
                <div class="form-row">
                    <div class="col-md-4">
                        <button type="button" data-toggle="collapse"
                                href="#messageForm{{$message->id}}" aria-expanded="false"
                                aria-controls="messageForm{{$message->id}}"
                                class="btn mb-1 btn-primary btn-xs btn-edit" id="btn-edit"
                                style="width: 100px;">Редагувати
                        </button>
                ...
                <div class="basic-form">
                    <div id="messageForm{{$message->id}}" class="collapse">
                        <br>
                        <form
                            action="{{ route($roleData['roleData']['messages_update_message'], ['id'=>$message->id]) }}"
                            method="post"
...
<div class="card-tools">
    {{ $privatChatMessages->links('pagination::bootstrap-4') }}
</div>
