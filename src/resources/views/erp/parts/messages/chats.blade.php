        ...
        <div class="card-body">
            <div class="card-content">
                ...
                @foreach($combinateIds as $userId)
                    ...
                    <a href="{{ route($roleData['roleData']['messages_show_private_chat'], ['id'=>$userId]) }}" class="alert-link" style="position: relative; display: block;">
                        <div class="alert alert-{{ $randomColor }}" style="position: relative; padding-right: 40px;">
                            <img src="{{ asset($user->photo_path) }}" height="40" width="40" class="mr-2 rounded-circle" alt="">
                            {{$user->name}} {{$user->lastname}}
                            @if($count > 0)
                                <span class="badge badge-pill gradient-1" style="position: absolute; top: -5px; right: -5px;">{{$count}}</span>
                            @endif
                        </div>
                    </a>
