            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-header">
                        <h3 class="card-title"><b>Вихiднi листи</b></h3>
                        <div class="card-tools">
                            <form action="{{ route($roleData['roleData']['emails_search']) }}" method="GET">
                                <div class="input-group">
                                    ...
                                    <input type="hidden" name="search_type" value="send_emails">
                                    <div class="append">
                                        <button class="btn btn-outline-dark rounded-1" type="submit"
                                                style="height: 40px;">
                                            Пошук
                                        </button>
                                        <a href="{{ route($roleData['roleData']['emails_send_emails']) }}"
                    ...
                    <form id="emailsdelete" action="{{ route($roleData['roleData']['emails_delete_email']) }}"
                          method="POST">
                        @method('PUT')
                        @csrf
                                ...
                                @foreach($mails as $mail)
                                    ...
                                    <tr>
                                        ...
                                        <td class="mailbox-name"><a
                                                href="{{ route($roleData['roleData']['emails_open_send_email'], ['id' => $mail->id]) }}">{{ $mail->recipient_email }}</a>
                                        </td>
                                        <td class="mailbox-subject">{{ $mail->subject }}</td>
                                        @if($mail->attachment_paths && $mail->attachment_paths !== 'null')
